...
import { submitSignIn } from '../../../config/api/services/authentication';
import LoadingModal from '../../reusables/Loading/Loading';
import TextButton from '../../reusables/Button/TextButton';
import { PasswordField } from '../../reusables/Field/PasswordField';
import { IconField } from '../../reusables/Field/IconField';
import AlertError from '../../reusables/Alert/Error';
import AlertSuccess from '../../reusables/Alert/Success';
        ...
        try {
            ...
            const res = await submitSignIn({
                payload: {
                    email,
                    password ,
                    roleType: process.env.REACT_APP_ROLE_TYPE,
                }
            });

            sessionStorage.setItem("accessToken", res.accessToken);
            sessionStorage.setItem("userData", JSON.stringify(res.user));
            setMessage({ success: "Berhasil masuk", error: '' });
            setLoading(false);
            router('/')
        } catch (error : any) {
                ...
                <div className="flex flex-wrap items-center">
                    <div className="hidden w-full xl:block xl:w-1/2">
                        <div className="py-17.5 px-26 text-center">
                            ...
                            <p className="2xl:px-20 text-body">
                                Selamat datang di dashboard Admin Kasiedu
                            </p>
                    ...
                    <div className="w-full border-stroke dark:border-strokedark xl:w-1/2 xl:border-l-2">
                        <div className="w-full p-4 sm:p-12.5 xl:p-17.5">
                            ...
                            {
                                message?.success ? <AlertSuccess message={message?.success} onClose={() => setMessage({ error: '', success: '' })} /> : null
                            }
                            {
                                message?.error ? <AlertError message={message?.error} onClose={() => setMessage({ error: '', success: '' })} /> : null
                            }
                            ...
                            <div>
                                ...
                                <PasswordField placeholder={"*******"} label={"Password"} value={password} onChange={(e : any) => setPassword(e.target.value)} />

                                <div className="mb-5">
                                    <TextButton title={"Sign In"} onClick={() => submit()} disable={false} />
                                </div>
                            </div>
                        </div>
                    </div>
