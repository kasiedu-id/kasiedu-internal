    ...
    return (
        <div>
            ...
            <UploadVocationCsvFile
                ...
                onAccept={() => {
                    navigate(`/vocations?page=1&limit=${query?.get('limit') || 20}`);
            ...
            <div className="p-4">
                ...
                <div className="flex justify-between items-center gap-10 mb-4">
                    ...
                    <div className="flex gap-3 pt-2">
                        <TextButton
                            title="Search"
                            onClick={() => navigate(`/vocations?page=1&limit=${query?.get('limit') || 20}&name=${name}&code=${code}`)}
                            disable={false}
                        />
                        <TextButton
                            title="Reset"
                            onClick={() => navigate(`/vocations?page=1&limit=${limit}&name=&code=`)}
                            disable={false}
                        />
                    </div>
                </div>
            ...
            <div className="flex flex-rows justify-between px-4 mt-5">
                {
                    page > 1 ? <p className="cursor-pointer font-semibold" onClick={() => navigate(`/vocations?page=${(Number(query?.get('page') || 2) - 1) || 1}&limit=${query?.get('limit') || 20}&name=${name}&code=${code}`)}>Previous</p> : <div></div>
                }
                {
                    totalCount > 0 ? <p>Page {page} of {Math.ceil(totalCount / limit)}</p> : <div></div>
                }
                {
                    page < totalCount / limit ? <p className="cursor-pointer font-semibold" onClick={() => navigate(`/vocations?page=${(Number(query?.get('page') || 1) + 1) || 1}&limit=${query?.get('limit') || 20}&name=${name}&code=${code}`)}>Next</p> : <div></div>
                }
            </div>
        </div>
    );
